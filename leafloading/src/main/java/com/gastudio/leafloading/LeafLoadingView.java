...
public class LeafLoadingView extends View {
    ...
    // 叶子旋转一周需要的时间
    private static final long LEAF_ROTATE_TIME = 1000;
    ...
    // 产生出的叶子信息
    private LinkedList<Leaf> mLeafInfos;
    // 用于控制随机增加的时间不抱团
    private int mAddTime;
    private boolean stopInvalidate= false;

    private Context mContext;
    private int lastProgressPostion;
    ...
    public LeafLoadingView(Context context, AttributeSet attrs) {
        ...
        mLeftMargin = UiUtils.dipToPx(context, LEFT_MARGIN);//左右margin
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(mOuterBitmap, mOuterSrcRect, mOuterDestRect, mBitmapPaint);//背景图在上
...
//        invalidate();
    ...
    private void drawProgressAndLeafs(Canvas canvas) {
        ...
        if (mProgress >= TOTAL_PROGRESS) {
            ...
            mContext.sendBroadcast(new Intent("com.gastudio.leafloading.stopInvalidate"));
//            return;
        }
        ...
        if (mCurrentProgressPosition < mArcRadius) {
            Log.i(TAG, "1111mProgress = " + mProgress + "---mCurrentProgressPosition = "
...
//            mWhiteRectF.left = mArcRightLocation;
            ...
            // 绘制叶子
            int localX = drawLeafs(canvas);
            //当叶子和进度条接触到，进度条增加
            if (localX == -1 || localX > lastProgressPostion) mCurrentProgressPosition = lastProgressPostion;
            else {
                mProgress += 1;
                lastProgressPostion = mCurrentProgressPosition;
                Leaf leaf = mLeafInfos.removeFirst();
                leaf.startTime = mLeafRotateTime +1;
                mLeafInfos.addLast(leaf);
            }

            if (lastProgressPostion == 0) return;
            Log.i(TAG, "localX"+localX+"___lastProgressPostion = " + lastProgressPostion + "---mCurrentProgressPosition = "
                    + mCurrentProgressPosition);
            // 3.绘制棕色 ARC
            // 单边角度
            ...
            Log.i(TAG, "22222mProgress = " + mProgress + "---transfer-----mCurrentProgressPosition = "
                    + mCurrentProgressPosition
...
//            mOrangeRectF.left = mArcRightLocation;
     ...
     * @param canvas
     */
    private int drawLeafs(Canvas canvas) {
        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (currentTime > leaf.startTime && leaf.startTime != 0) {
                ...
                int localX = getLeafLocation(leaf, currentTime);
                if (i == 0) firstLeftX = localX;
                // 根据时间计算旋转角度
                canvas.save();
                // 通过Matrix控制叶子旋转
...
//                Log.i(TAG, "left.x = " + leaf.x + "--leaf.y=" + leaf.y);
                ...
                int angle = (int) (rotateFraction * 360);//随时间消逝增加旋转变换
                // 根据叶子旋转方向确定叶子旋转角度
    ...
    private int getLeafLocation(Leaf leaf, long currentTime) {
        ...
        if (intervalTime < 0) {
            ...
            leaf.startTime = System.currentTimeMillis()
                    + new Random().nextInt((int) mLeafFloatTime);//始终保持当前时间与叶子开始出现时间的间隔在一个周期内
        }

        float fraction = (float) intervalTime / mLeafFloatTime;//叶子时间比率
        leaf.x = (int) (mProgressWidth - mProgressWidth * fraction);//叶子距离左侧的坐标
    ...
    private int getLocationY(Leaf leaf) {
        // y = A(wx+Q)+h
        float w = (float) ((float) 2 * Math.PI / mProgressWidth);//计算系数
        float a = mMiddleAmplitude;//初始化振幅。默认中等振幅
        switch (leaf.type) {
            case LITTLE:
                // 小振幅 ＝ 中等振幅 － 振幅差
...
//        Log.i(TAG, "---a = " + a + "---w = " + w + "--leaf.x = " + leaf.x);
        return (int) (a * Math.sin(w * leaf.x)) + mLeftMargin;
    }
    ...
    private void initBitmap() {
        mLeafBitmap = ((BitmapDrawable) mResources.getDrawable(R.drawable.leaf)).getBitmap();//拿到叶子位图，取得宽高
        mLeafWidth = mLeafBitmap.getWidth();
        mLeafHeight = mLeafBitmap.getHeight();

        mOuterBitmap = ((BitmapDrawable) mResources.getDrawable(R.drawable.leaf_kuang)).getBitmap();//拿到外框背景位图，取得宽高
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mTotalWidth = w;
        mTotalHeight = h;
        mProgressWidth = mTotalWidth - mLeftMargin - mRightMargin;//总进度条宽度
        mArcRadius = (mTotalHeight - 2 * mLeftMargin) / 2;//去掉上下两个边框除以2得到半径

        mOuterSrcRect = new Rect(0, 0, mOuterWidth, mOuterHeight);//源
        mOuterDestRect = new Rect(0, 0, mTotalWidth, mTotalHeight);//目标

        mWhiteRectF = new RectF(mLeftMargin + mCurrentProgressPosition, mLeftMargin, mTotalWidth
                - mRightMargin,
                mTotalHeight - mLeftMargin);//白色矩形区域定界
        mOrangeRectF = new RectF(mLeftMargin + mArcRadius, mLeftMargin,
                mCurrentProgressPosition
                , mTotalHeight - mLeftMargin);//橙色矩形区域定界

        mArcRectF = new RectF(mLeftMargin, mLeftMargin, mLeftMargin + 2 * mArcRadius,
                mTotalHeight - mLeftMargin);//圆弧矩形区域定界
    ...
    private class LeafFactory {
        private static final int MAX_LEAFS = 8;//最多叶子数量
        ...
        public Leaf generateLeaf() {
            ...
            mLeafFloatTime = mLeafFloatTime <= 0 ? LEAF_FLOAT_TIME : mLeafFloatTime;
            mAddTime += random.nextInt((int) (mLeafFloatTime * 2));//通过漂浮时间取得叶子开始出现时间
            leaf.startTime = System.currentTimeMillis() + mAddTime;
            return leaf;
        }

        // 根据最大叶子数产生叶子信息
        public LinkedList<Leaf> generateLeafs() {
            return generateLeafs(MAX_LEAFS);
        }

        // 根据传入的叶子数量产生叶子信息
        public LinkedList<Leaf> generateLeafs(int leafSize) {
    ...
    public void setStopInvalidate(boolean stopInvalidate){
        this.stopInvalidate = stopInvalidate;
    }
    /**
     * 设置进度
     * 
